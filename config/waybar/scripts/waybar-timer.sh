...
if [ "$cmd" == "check" ]; then
    ...
    if [ "$DATE_TIME_TARGET" == "READY" ]; then
        echo '{"text": "'"<span font='13' rise='-2000'>󰔛</span> "'", "tooltip": "Timer is not active"}'

    elif [ "$DATE_TIME_TARGET" == "FINISHED" ]; then
        echo "READY" > "$TFILE"
        zenity --info --no-wrap --text="Timer expired!"
        echo '{"text": "'"<span font='13' rise='-2000'>󰔛</span> "'"}'
        ...
        echo '{"text": "'"<span font='13' rise='-2000'>󰔟</span> $REMAINING"'", "class": "active", "tooltip": "'"$DATE_TIME_TARGET_TITLE\n\n$DATE_TIME_TARGET_SHORT"'"}'
    else
        if [ -f "$TFILE" ]; then
            echo "FINISHED" > "$TFILE"
        else
