...
IMAGE="${IMAGE:-docker.io/opensuse/tumbleweed:latest}"
CONTAINER_NAME="${CONTAINER_NAME:-dotfiles-opensuse-test}"
CONTAINER_USER="${CONTAINER_USER:-dev}"
CONTAINER_PASSWORD="${CONTAINER_PASSWORD:-password}"
CUSTOM_IMAGE="${CUSTOM_IMAGE:-dotfiles-opensuse-user}"
...
podman build \
    ...
    -t "$CUSTOM_IMAGE" \
    -f "$ROOT_DIR/system/Containerfile.opensuse-test" \
    "$ROOT_DIR" >/dev/null
