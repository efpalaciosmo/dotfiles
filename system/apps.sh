...
installFNM() {
    ...
    if command -v fnm >/dev/null 2>&1; then
        ...
        printf "%b\n" "${YELLOW}Note: fnm was installed without shell auto-activation${RC}"
    fi
}
...
installGolang() {
    ...
    if command -v go >/dev/null 2>&1 && go version | grep -q "go1.26.1"; then
        ...
        go_tarball="$HOME/.local/downloads/go1.26.1.linux-${go_arch}.tar.gz"
        go_url="https://go.dev/dl/go1.26.1.linux-${go_arch}.tar.gz"
        ...
        if command -v wget >/dev/null 2>&1; then
            ...
            printf "%b\n" "${RED}Neither wget nor curl is available to download Go${RC}"
            exit 1
        fi
...
main() {
    ...
    printf "%b\n" "${YELLOW}  • fnm (Node version manager) installed${RC}"
