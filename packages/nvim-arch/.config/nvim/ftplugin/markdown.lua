-- ============================================================================
-- ftplugin/markdown.lua (plugin-free)
--
-- Visual rendering on this profile uses Neovim's built-in conceal +
-- the standard markdown syntax shipped in $VIMRUNTIME. There is NO
-- render-markdown.nvim, so we lean on:
--   * conceallevel = 2 / concealcursor = "nc"   (hides ** and _)
--   * spell + linebreak                          (prose ergonomics)
--
-- This file also carries the editor helpers that act on the BUFFER
-- (toggle bullets / numbers / checkboxes / task state / heading level).
-- ============================================================================
...
-- Toggle conceal locally (replaces `<leader>mr` from render-markdown).
